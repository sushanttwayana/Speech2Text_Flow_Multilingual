...
async def translate_to_english(audio_np_segment: np.ndarray, language: str) -> str:
    """
    Translates speech directly to English using Whisper's translation capability.
    """
    if not MODEL_LOADED:
        logger.error("Translation requested but model not loaded.")
        raise RuntimeError("Model not loaded.")
    ...
    try:
        ...
        # If already English, just transcribe
        if language == "en":
            generate_kwargs = {
                "language": "english",
                "task": "transcribe",
...
async def transcribe_segment_dual(audio_np_segment: np.ndarray, language: str) -> Dict[str, str]:
    ...
    if language == "en":
        # For English, just transcribe once
        english_text = await transcribe_segment_native(audio_np_segment, language)
        native_text = english_text
    else:
        # For non-English, get both native and translated versions
        native_text = await transcribe_segment_native(audio_np_segment, language)
        english_text = await translate_to_english(audio_np_segment, language)
...
# ---------- END ENHANCED MULTILINGUAL TRANSCRIPTION ----------
