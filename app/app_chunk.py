...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        # Update the local_dir as per your actual model path
        ...
        NLU_MODEL_PATH = "models/nlu_new_trained.tar.gz"
...
async def transcribe_segment(audio_np_segment: np.ndarray, language: str) -> str:
    ...
    try:
        ...
        # Transcription with adjusted parameters (no initial_prompt because it's unsupported here)
        generate_kwargs = {
            "language": language,
            "temperature": 0.0,
            "num_beams": 5,
...
async def transcribe_full_audio(client_id: str, full_audio: np.ndarray, language: str) -> str:
    """Final pass over full audio with max 30s chunks for best accuracy."""
    ...
    try:
        while True:
            ...
            if data["type"] == "stop_recording":
                ...
                # Merge interim transcriptions
                final_segments = manager.transcribed_segments.get(client_id, [])

                # Run longer final pass for accuracy on the entire recorded audio
                if final_audio.size > 0:
                    final_full_transcription = await transcribe_full_audio(client_id, final_audio, client_languages.get(client_id, "en"))
                    if final_full_transcription:
                        logger.info(f"Final full transcription for {client_id}: {final_full_transcription}")
                        # Replace chunked transcriptions with the more accurate full one to send
                        final_segments = [final_full_transcription]

                final_text = merge_transcriptions(final_segments) or "No speech detected."
                logger.info(f"Final transcription (merged) for {client_id}: {final_text}")

                # Initialize result to prevent errors
                result = {"action": "error", "message": "No transcription or Rasa processor not initialized.", "form_data": {}}

                # Process with Rasa intent classifier
                ...
                # Navigate with prefill for topup_wallet intent
